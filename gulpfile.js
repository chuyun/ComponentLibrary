...
gulp.task('watch', function () {
    ...
    gulp.watch('Component/**/*.scss', ['compass']);
    gulp.watch('Component/**/*.css', browserSync.reload);
});
...
gulp.task('spriter_param', function () {
    ...
    return gulp.src('./css/main.css') //xxx.css是你的css文件，在图片url后增加特定参数(?__sprite)才去处理雪碧图
        .pipe(spriter_param({
            // 生成的sprite图片存放地址
            'spriteSheet': './dist/sprite-01.png',
            // Because we don't know where you will end up saving the CSS file at this point in the pipe,
            // we need a litle help identifying where it will be.
            'pathToSpriteSheetFromCSS': '../css/sprite-01.png' //这是在css引用的图片路径，很重要
        }))
        .pipe(gulp.dest('./dist2')); //最后生成出来
});
/**
